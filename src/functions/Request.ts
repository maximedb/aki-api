...
type checkParamProperty = 'elements' | 'answers' | 'identification' | 'element_informations' | 'name';
...
export const requestReport = async (url: string, checkParamProperty: checkParamProperty, region: region, config: configOptions): Promise<AkinatorAPIError | AkinatorResult> => {
  const axiosConfig = (config || {}) as AxiosRequestConfig;
